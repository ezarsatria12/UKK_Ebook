...
<body>
    <body>
        <div class="folmulir">
            ...
            <form class="form-card" action="/dasboard/{{ $book->id}}" method="post" enctype="multipart/form-data">
                @method('put')
                @csrf
                <div class="row justify-content-between text-left">
                    <div class="form-group col-sm-6 flex-column d-flex"> <label class="form-control-label px-3">Judul Buku<span class="text-danger"> *</span></label> <input class="@error('judul')
                        is_invalid
                    @enderror" type="text" id="judul" placeholder="" name="judul" autofocus required value="{{ old('judul', $book->judul)}}"> </div>
                    <div class="form-group col-sm-6 flex-column d-flex"> <label class="form-control-label px-3">Penulis<span class="text-danger"> *</span></label> <input class="@error('penulis')
                        is_invalid
                    @enderror" type="text" id="penulis" placeholder="" name="penulis" autofocus required value="{{ old ('penulis', $book->penulis)}}"> </div>
                </div>
                <div class="row justify-content-between text-left">
                    <div class="form-group col-sm-6 flex-column d-flex"> <label class="form-control-label px-3">Penerbit<span class="text-danger"> *</span></label> <input class="@error('penerbit')
                        is_invalid
                    @enderror" type="text" id="penerbit"  placeholder="" name="penerbit" autofocus required value="{{ old ('penerbit', $book->penerbit) }}"> </div>
                    <div class="form-group col-sm-6 flex-column d-flex"> <label class="form-control-label px-3">Tahun Terbit<span class="text-danger"> *</span></label> <input class="@error('tahunterbit')
                        is_invalid
                    @enderror" type="number" id="tahunterbit"  placeholder="Tahun" name="tahunterbit" autofocus required value="{{ old ('tahunterbit', $book->tahunterbit) }}"> </div>
                </div>
                <div class="row justify-content-between text-left">
                    <div class="form-group col-sm-6 flex-column d-flex"> 
                        <label class="form-control-label px-3">Pilih Gambar<span class="text-danger"> *</span></label>
                        
                        <img src="{{ asset('storage/'.$book->gambar)}}" class="img-preview img-fluid  col-sm-6" >
                    
                        <div class="input-group">
                            <div class="form-group">
                                <input type="file" class="form-control-file" id="exampleFormControlFile1" name="gambar" autofocus required value="{{ old ('storage/'.$book->gambar) }}" onchange="previewImage()">
                              </div>
                        </div> 
                    </div>
                </div>
